...
## 🧭 DevGraph — репозиторий как режим OmniSight

OmniSight теперь может анализировать репозитории как ещё один режим платформы:
...
DevGraph делает репозиторий интерактивным объектом: пользователь загружает репозиторий, а ИИ автоматически:

- строит граф модулей и импортов;
- понимает архитектуру и границы модулей;
- находит циклические зависимости;
- показывает hotspots (узлы с высокой степенью важности);
- предоставляет объяснения кода и контекст через LLM;
- ищет потенциально мёртвый код;
- отвечает на вопросы по коду и предлагает рефакторинги.

Архитектура модуля DevGraph (скелет в `parser/`, `graph/`, `ai/`, `api/devgraph.py`, `frontend/`):

```
omnisight/
│
├── parser/     # языковые парсеры (python, typescript, java...)
├── graph/      # построение графа, анализ, метрики, алгоритмы
├── ai/         # RAG, embeddings, llm explainers
├── frontend/   # DevGraph UI (React + Three.js)
└── api/        # FastAPI endpoints (анализ, запросы, результаты)
```

Это позволит развивать DevGraph не как отдельный инструмент, а как полнофункциональный режим OmniSight — расширяемый, мульти-языковой и интегрированый с AI-помощником.

Если хотите, могу:

- развернуть front-end прототип (React + visx/three) для визуализации;
- добавить анализатор TypeScript и Java;
- интегрировать RAG/LLM для объяснений и рефакторинг-предложений;
- подготовить CI (tests + security checks) и пример workflow для GitHub Actions.
